...
local UI = { Mobile = isMobile }

-- Rayfield is the only external UI dependency. Keep the feature code below
-- independent from Rayfield's exact control object names.
local Rayfield
...
if rayfieldOk then
    ...
    warn("[Kinginul] Failed to load Rayfield:", rayfieldResult)
end
...
if Rayfield then
    ...
    Window = Rayfield:CreateWindow({
        Name = "Kinginul | Violence District v2.0.0",
        LoadingTitle = "Kinginul",
    ...
    local function controlWrapper(control, flag)
        local wrapper = control or {}
        if flag then Window.ConfigElements[flag] = wrapper end
        return wrapper
    end

    local function createControl(host, methodName, cfg, flag)
        if host and type(host._ensureSection) == "function" then
            host._ensureSection()
        end
        local creator = host and host[methodName]
        if type(creator) ~= "function" then return controlWrapper(nil, flag) end
        local okControl, control = pcall(function() return creator(host, cfg) end)
        return controlWrapper(okControl and control or nil, flag)
    end

    local function makeRayfieldHost(tab, lazySectionName)
        local host = {}
        local sectionCreated = not lazySectionName
        function host._ensureSection()
            if sectionCreated then return end
            if tab.CreateSection then
                pcall(function() tab:CreateSection(lazySectionName) end)
            end
            sectionCreated = true
        end
        function host:AddSection(cfg)
            host._ensureSection()
            if tab.CreateSection and cfg and cfg.Name then pcall(function() tab:CreateSection(cfg.Name) end) end
            return makeRayfieldHost(tab, lazySectionName)
        end
                function host:AddCenterTabbox(name)
            local box = {}
            local groupPrefix = name or "Features"

            function box:AddTab(cfg)
                local childHost = makeRayfieldHost(tab, cfg and cfg.Name)

                -- Wrap AddSection supaya nama section di-prefix dengan nama grup
                local originalAddSection = childHost.AddSection
                childHost.AddSection = function(self, sectionCfg)
                    sectionCfg = sectionCfg or {}
                    if sectionCfg.Name and sectionCfg.Name ~= "" then
                        sectionCfg.Name = groupPrefix .. "  ·  " .. sectionCfg.Name
                    else
                        sectionCfg.Name = groupPrefix
                    end
                    return originalAddSection(self, sectionCfg)
                end
        ...
        function host:AddDivider()
            host._ensureSection()
            if tab.CreateDivider then pcall(function() tab:CreateDivider() end) end
            return host
        end    
        function host:AddToggle(cfg)
            local options = { Name = cfg.Name, CurrentValue = cfg.Default == true, Flag = cfg.Flag or cfg.Name, Callback = cfg.Callback }
            return createControl(tab, "CreateToggle", options, options.Flag)
        end
        function host:AddSlider(cfg)
            local options = { Name = cfg.Name, Range = { cfg.Min or 0, cfg.Max or 100 }, Increment = cfg.Increment or 1, CurrentValue = cfg.Default or cfg.Min or 0, Flag = cfg.Flag or cfg.Name, Callback = cfg.Callback }
            return createControl(tab, "CreateSlider", options, options.Flag)
        end        
        function host:AddDropdown(cfg)
            local options = { Name = cfg.Name, Options = cfg.Values or {}, CurrentOption = cfg.Default, MultipleOptions = cfg.Multi == true, Flag = cfg.Flag or cfg.Name, Callback = cfg.Callback }
            return createControl(tab, "CreateDropdown", options, options.Flag)
        end
        function host:AddButton(cfg)
            return createControl(tab, "CreateButton", { Name = cfg.Name, Callback = cfg.Callback }, nil)
        end
        function host:AddKeybind(cfg)
            local options = { Name = cfg.Name, CurrentKeybind = cfg.Default or "None", HoldToInteract = false, Flag = cfg.Flag or cfg.Name, Callback = cfg.Callback }
            return createControl(tab, "CreateKeybind", options, options.Flag)
        end
        function host:AddColorPicker(cfg)
            local options = { Name = cfg.Name, Color = cfg.Default or Color3.new(1, 1, 1), Flag = cfg.Flag or cfg.Name, Callback = cfg.Callback }
            return createControl(tab, "CreateColorPicker", options, options.Flag)
        end
        function host:AddTextInput(cfg)
            local options = { Name = cfg.Name, CurrentValue = cfg.Default or "", PlaceholderText = cfg.Placeholder or "", RemoveTextAfterFocusLost = false, Flag = cfg.Flag or cfg.Name, Callback = cfg.Callback }
            return createControl(tab, "CreateInput", options, options.Flag)
        end
        function host:AddParagraph(cfg)
            return createControl(tab, "CreateParagraph", { Title = cfg.Title or cfg.Name, Content = cfg.Content or cfg.Text or "" }, nil)
        end
        function host:AddLabel(cfg)
            return host:AddParagraph({ Title = cfg.Name or cfg.Text, Content = cfg.Text or cfg.Name or "" })
        end
        return host
    end

    function Window:AddTab(cfg)
        return makeRayfieldHost(Window:CreateTab(cfg.Name))
    end
    ...
    function Window:SetAccount()
    end

    function Window:Toggle()
        return Rayfield:ToggleUI()
    end
    ...
    local KinginulUIVisible = true
    local function SetKinginulUIVisible(visible)
        KinginulUIVisible = visible and true or false
        local handled = false

        for _, methodName in ipairs({ "SetVisible", "SetVisibility" }) do
            if type(Window[methodName]) == "function" then
                handled = pcall(Window[methodName], Window, KinginulUIVisible)
                if handled then break end
            end
        end

        if not handled and type(Window.Toggle) == "function" then
            handled = pcall(Window.Toggle, Window)
        end

        if not handled then
            for _, root in ipairs({ Window.Gui, Window.GUI, Window.ScreenGui, Window.Main, Window.Container }) do
                if typeof(root) == "Instance" then
                    if root:IsA("ScreenGui") then
                        root.Enabled = KinginulUIVisible
                        handled = true
                    elseif root:IsA("GuiObject") then
                        root.Visible = KinginulUIVisible
                        handled = true
                    end
                end
                if handled then break end
    ...
    UserInputService.InputBegan:Connect(function(input)
        if input.KeyCode == Enum.KeyCode.RightControl then
            SetKinginulUIVisible(not KinginulUIVisible)
        end
    end)
end
...
-- =====================================================
-- CENTRALIZED METAMETHOD HOOK DISPATCHER
-- Registration order: AntiFail, AntiBlind, NoCutscene,
-- AntiStunSlowdown, VeilInterceptor.
-- Handlers run top-to-bottom in that order. BLOCK returns nil and stops.
-- PASS continues. {args...} replacement is terminal and calls the original.
-- =====================================================
local KYS_Hooks = getgenv().KYS_Hooks or {}
KYS_Hooks._handlers = KYS_Hooks._handlers or {
    namecall = {},
    index = {},
    newindex = {},
}
KYS_Hooks._installed = KYS_Hooks._installed or false
KYS_Hooks._originals = KYS_Hooks._originals or {}
KYS_Hooks._registrationOrder = {
    AntiFail = 1,
    AntiBlind = 2,
    NoCutscene = 3,
    AntiStunSlowdown = 4,
...
local function KYS_HooksSortHandlers(hookType)
    table.sort(KYS_Hooks._handlers[hookType], function(left, right)
        return (KYS_Hooks._registrationOrder[left.Name] or math.huge)
            < (KYS_Hooks._registrationOrder[right.Name] or math.huge)
    end)
end

for hookType in pairs(KYS_Hooks._handlers) do
    KYS_HooksSortHandlers(hookType)
end

function KYS_Hooks.Register(hookType, name, handlerFn)
    if (hookType ~= "namecall" and hookType ~= "index" and hookType ~= "newindex")
        or type(name) ~= "string" or type(handlerFn) ~= "function" then
        return false
    end
    ...
    table.insert(KYS_Hooks._handlers[hookType], {
        Name = name,
        Fn = handlerFn,
    })
    KYS_HooksSortHandlers(hookType)
...
function KYS_Hooks.Return(value)
    return { __KYS_HookReturn = true, Value = value }
end
...
local function KYS_HooksDispatch(hookType, self, ...)
    ...
    for _, handler in ipairs(KYS_Hooks._handlers[hookType]) do
        ...
        elseif type(result) == "table" then
            ...
            return original(self, unpack(result))
        end
    end
...
local function KYS_SafeCClosure(fn)
    if type(newcclosure) == "function" then
        ...
        warn("[KYS_Hooks] newcclosure threw, falling back to raw fn")
    else
        warn("[KYS_Hooks] newcclosure missing (type=" .. tostring(type(newcclosure)) .. "), using raw fn")
    end
    return fn
end

function KYS_Hooks.InstallAll()
    if KYS_Hooks._installed then return end

    local ok, err = pcall(function()
        KYS_Hooks._originals.namecall = hookmetamethod(game, "__namecall", KYS_SafeCClosure(function(self, ...)
            return KYS_HooksDispatch("namecall", self, ...)
        end))
        getgenv().KYS_oldNamecall = KYS_Hooks._originals.namecall

        KYS_Hooks._originals.index = hookmetamethod(game, "__index", KYS_SafeCClosure(function(self, ...)
            return KYS_HooksDispatch("index", self, ...)
        end))
        KYS_Hooks._originals.newindex = hookmetamethod(game, "__newindex", KYS_SafeCClosure(function(self, ...)
            return KYS_HooksDispatch("newindex", self, ...)
        end))
    end)
    if ok then
        KYS_Hooks._installed = true
    else
        warn("[KYS_Hooks] InstallAll failed:", err)
    end
end

getgenv().KYS_Hooks = KYS_Hooks
print("[KYS_Hooks] env check:",
    "hookmetamethod=" .. tostring(type(hookmetamethod)),
    "newcclosure=" .. tostring(type(newcclosure)),
    "hookfunction=" .. tostring(type(hookfunction)),
    "getrawmetatable=" .. tostring(type(getrawmetatable)))
KYS_Hooks.InstallAll()
...
-- → infinite recursion → stack overflow → force close.
            ...
            KYS_Hooks.Register("namecall", "AntiFail", function(self, ...)
...
function veil_setupInterceptor()
    ...
    task.spawn(function()
        pcall(function()
            KYS_Hooks.Register("namecall", "VeilInterceptor", function(self, ...)
                local args = {...}
                local method = getnamecallmethod()
                if not checkcaller() then
                    ...
                    if method == "GetAttribute" then
                        if args[1] == "LakeMist" and VD.KILLER_InfLakeMist then
                            ...
                            if caller and caller.Name == "AwardLog" then return KYS_Hooks.Return(0) end
                            return KYS_Hooks.Return(false)
                        end
                    ...
                    if method == "GetAttributes" then
                        if VD.KILLER_InfLakeMist or VD.KILLER_InfPursuit then
                            local attrs = KYS_Hooks._originals.namecall(self, ...)
...
game:GetService("RunService").RenderStepped:Connect(function()
    local cam = workspace.CurrentCamera
    if not cam or typeof(cam) ~= "Instance" then return end
    ...
    if VeilState.chargingSpear and VeilConfig.Enabled and isSpearMode then
        ...
        if target and target.Part and target.Part.Parent and typeof(target.Part.Parent) == "Instance" then
            if VeilDraw.Highlight and typeof(VeilDraw.Highlight) == "Instance" then
                pcall(function()
                    VeilDraw.Highlight.Parent = target.Part.Parent
                    VeilDraw.Highlight.Adornee = target.Part
                end)
            end

            if VeilConfig.ShowTargetLaser then
                if not getgenv().KYS_SpearLaserPart or not getgenv().KYS_SpearLaserPart.Parent then
                    local laser = Instance.new("Part")
...
if Window then
    ...
    local function makeRayfieldFeatureAdapter(section)
        local adapter = {}
        local function createNamedSection(cfg)
            if not (cfg and cfg.Name) then
                return adapter
            end

            if section and type(section.CreateSection) == "function" then
                local createdSection
                local ok, result = pcall(function()
                    return section:CreateSection(cfg.Name)
                end)
                if ok and result then
                    return makeRayfieldFeatureAdapter(result)
                end
            end

            if section and type(section.AddDivider) == "function" then
                ...
                if k == "AddSlider" then
                    return function(self, cfg)
                        if cfg and cfg.Name then
                            ...
                            local isFloat = false
                            if rayfieldCfg.Increment < 1 or (math.floor(rayfieldCfg.Min) ~= rayfieldCfg.Min) or (math.floor(rayfieldCfg.Max) ~= rayfieldCfg.Max) then
                                isFloat = true
                            end
                            if isFloat then
                                rayfieldCfg.Rounding = 1
                                if rayfieldCfg.Increment <= 0.01 then
                                    rayfieldCfg.Rounding = 2
                                end
                            else
                                rayfieldCfg.Rounding = 0
                            end
                            rayfieldCfg.Callback = function(Value)
                                if cfg.Callback then
                                    pcall(function() cfg.Callback(Value) end)
                                end
                            end
                            pcall(function() section:AddSlider(rayfieldCfg) end)
                        end
                        return adapter
                    end
                end
    ...
    local Tabs = {
        ...
        ScriptSettings = Window:AddTab({ Name = "s-settings", Icon = "lucide:settings", Type = "Single" }),
    }

    local function KYS_KillScript()
        if VD.Destroyed then return end
        VD.Destroyed = true
        getgenv().KYS_ScriptKilled = true
        ...
        safeCall("KYS_VD_VisualESP_Cleanup")
        safeCall("KYS_SetHideSurvivorIcon", false)
        safeCall("KYS_SetShowHookCounter", false)
        safeCall("KYS_SetShowPingFPS", false)
        safeCall("KYS_SetToFSilentAim", false)
        safeCall("KYS_SetFlashlightSilentAim", false)
        safeCall("StopKystKiller")
        safeCall("StopSpectatorCounter")
        safeCall("StopKillerPerksDisplay")
        safeCall("StopPredictMap")
        safeCall("VD_DisableNoclip")
        safeCall("VD_SetMoonwalkButtonVisible", false)
        safeCall("VD_SetAimLockButtonVisible", false)
        safeCall("RestoreFirstPersonCamera")
        ...
        pcall(function()
            if Window and type(Window.Destroy) == "function" then
                Window:Destroy()
            elseif Rayfield and type(Rayfield.Destroy) == "function" then
                Rayfield:Destroy()
            elseif Rayfield and type(Rayfield.ToggleUI) == "function" then
                Rayfield:ToggleUI()
            end
        end)

        print("[Kinginul] Script stopped. Re-execute to start again.")
    end

    local scriptSettingsSection = Tabs.ScriptSettings:AddSection({
        Position = "Center",
    ...
    scriptSettingsSection:AddParagraph({
        Title = "Debug Shutdown",
        Content = "Stops active loops, cleans script UI, and restores temporary state without leaving the server.",
    })
    scriptSettingsSection:AddButton({
        Name = "Kill Script",
        Callback = KYS_KillScript,
    })
                    ...
                    return function(self, cfg)
                        if cfg and cfg.Name then
                            if type(tab.CreateSection) == "function" then
                                local createdSection
                                local ok, result = pcall(function()
                                    return tab:CreateSection(cfg.Name)
                                end)
                                if ok and result then
                                    return makeRayfieldFeatureAdapter(result)
                                end
                        ...
                        return makeRayfieldFeatureAdapter(tab)
                    end
                end
                return tab[k]
            end
    ...
    local function addCenterFeatureTabbox(tab, name, entries)
        ...
        for _, entry in ipairs(entries) do
            created[entry.Key] = makeRayfieldFeatureAdapter(tabbox:AddTab({
                Name = entry.Name,
                Icon = entry.Icon,
            }))
        end
    ...
    PlayerMiscFeatureTabs = addCenterFeatureTabbox(Tabs.Player, "Player Misc", {
        ...
        { Key = "Avatar", Name = "Korless", Icon = "solar:users-group-rounded-bold" },
    })
...
do -- Generator Tab
    ...
    genAuto:AddButton({
        Name = "TP to Nearest Incomplete Gen",
        Callback = function()
            if getgenv().KYS_TP_NearestIncompleteGen then
                pcall(getgenv().KYS_TP_NearestIncompleteGen)
            else
                pcall(VD_Notify, "TP Generator", "Function belum siap, coba lagi nanti.", 3)
            end
        end
    })
    
    genAuto:AddButton({
        Name = "TP to Next Incomplete Gen (Cycle)",
        Callback = function()
            if getgenv().KYS_TP_NextIncompleteGen then
                pcall(getgenv().KYS_TP_NextIncompleteGen)
            else
                pcall(VD_Notify, "TP Generator", "Function belum siap, coba lagi nanti.", 3)
            end
        end
...
print("Kinginul Violence District v2.0.0 Loaded")
pcall(function()
    VD_Notify("Kinginul", "Violence District v2.0.0 Loaded Successfully!", 5)
end)
...
function KYS_TeleportToGenerator(index)
    if not KYS_Cache or not KYS_Cache.Generators or #KYS_Cache.Generators == 0 then print("[Kinginul] Generator tidak ditemukan") return false end
...
function KYS_TeleportToGate()
    if not KYS_Cache or not KYS_Cache.Gates or #KYS_Cache.Gates == 0 then print("[Kinginul] Gate tidak ditemukan") return false end
    local closest, closestDist = nil, math.huge
...
function KYS_TeleportToHook()
    if not KYS_Cache or not KYS_Cache.ClosestHook then print("[Kinginul] Hook tidak ditemukan") return false end
    return KYS_TeleportToPosition(KYS_Cache.ClosestHook.part.Position)
end

-- =====================================================
-- TP TO INCOMPLETE GENERATOR
-- =====================================================
local KYS_GenCycleIndex = 0

local function KYS_GetIncompleteGenerators()
    local list = {}
    for _, gen in ipairs(KYS_Cache.Generators or {}) do
        if gen.model and gen.model.Parent and gen.part then
            local pct = tonumber(gen.model:GetAttribute("RepairProgress"))
                or tonumber(gen.model:GetAttribute("ProgressRepair"))
                or tonumber(gen.model:GetAttribute("repairProgress"))
                or 0
            if pct >= 0 and pct <= 1.001 then pct = pct * 100 end
            
            -- Anggap belum selesai kalau progress < 99.5%
            if pct < 99.5 then
                table.insert(list, {
                    model = gen.model,
                    part = gen.part,
                    progress = pct,
                    dist = Root and (gen.part.Position - Root.Position).Magnitude or math.huge,
                })
            end
        end
    end
...
function KYS_TP_NearestIncompleteGen()
    local list = KYS_GetIncompleteGenerators()
    if #list == 0 then
        pcall(VD_Notify, "TP Generator", "Semua generator udah 100% bre!", 3)
        return
    end
    ...
    if KYS_TeleportToPosition(target.part.Position) then
        pcall(VD_Notify, "TP Generator",
            string.format("TP ke Gen (%.0f%%) - %d gen tersisa", target.progress, #list), 3)
    end
end

function KYS_TP_NextIncompleteGen()
    local list = KYS_GetIncompleteGenerators()
    if #list == 0 then
        pcall(VD_Notify, "TP Generator", "Semua generator udah 100% bre!", 3)
        return
    ...
    -- Sort by jarak biar cycling-nya konsisten
    table.sort(list, function(a, b) return a.dist < b.dist end)

    KYS_GenCycleIndex = (KYS_GenCycleIndex % #list) + 1
    local target = list[KYS_GenCycleIndex]

    if KYS_TeleportToPosition(target.part.Position) then
        pcall(VD_Notify, "TP Generator",
            string.format("Gen %d/%d (%.0f%%)", KYS_GenCycleIndex, #list, target.progress), 3)
    end
end

getgenv().KYS_TP_NearestIncompleteGen = KYS_TP_NearestIncompleteGen
getgenv().KYS_TP_NextIncompleteGen = KYS_TP_NextIncompleteGen
...
function SetupAntiBlind()
    pcall(function()
        ...
        local _genv = getgenv()
        KYS_Hooks.Register("namecall", "AntiBlind", function(self, ...)
            if not checkcaller() and _genv.VD and _genv.VD.KILLER_AntiBlind and self == gb then
                ...
                if method == "FireServer" and GetRole() == "Killer" then
                    return "BLOCK" -- Block pengiriman sinyal buta
                end
            end
            return "PASS"
        end)
...
function SetupAntiStunSlowdown()
    ...
    local _genv = getgenv()
    KYS_Hooks.Register("newindex", "AntiStunSlowdown", function(t, k, v)
        if k == "WalkSpeed" or k == "Anchored" then
            if not checkcaller() and _genv.VD and _genv.VD.KILLER_NoSlowdown and GetRole() == "Killer" then
                ...
                if k == "WalkSpeed" and typeof(v) == "number" and v < 16 and typeof(t) == "Instance" and t:IsA("Humanoid") then
                    return { k, _genv.VD.SPEED_Value or 16 }
                end
...
function SetupNoCutsceneHook()
    ...
    local _genv = getgenv()
    KYS_Hooks.Register("index", "NoCutscene", function(t, k)
        if k ~= "OnClientEvent" and k ~= "Event" then return "PASS" end
        if not (_genv.VD and _genv.VD.NoCutscene and not checkcaller() and typeof(t) == "Instance") then return "PASS" end
        local parent = t.Parent
        if not (parent and parent.Name == "Game") then return "PASS" end
        local name = t.Name
        if name == "cutscene" and k == "Event" then
            return KYS_Hooks.Return(fakeBindable.Event)
        end
        if name == "cutsceneEnd" or name == "cutsceneEnd2" or name == "cutsceneEndwithownchar" or name == "endscreencutscene" then
            return KYS_Hooks.Return(fakeRemote.OnClientEvent)
        end
        return "PASS"
    end)
end
...
function setupSpearAimbotBtn()
    ...
    print("[Kinginul] Spear Aimbot Button created successfully")
end
